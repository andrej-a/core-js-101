...
function timeSpanToString(startDate, endDate) {
  ...
  hours = hours > 9 ? Math.floor(hours) : `0${Math.floor(hours)}`;
  let minutes = (diff / 60000) % 60;
  minutes = minutes > 9 ? Math.floor(minutes) : `0${Math.floor(minutes)}`;
  let seconds = ((diff / 1000) % 60);
  seconds = Math.floor(seconds) > 9 ? Math.floor(seconds) : `0${Math.floor(seconds)}`;
...
function angleBetweenClockHands(date) {
  const previousDate = new Date(date); // get date
  let hours = previousDate.getUTCHours(); // get hours by UTC
  hours = hours > 12 ? hours - 12 : hours;
  const minutes = previousDate.getUTCMinutes(); // get minutes by UTC
